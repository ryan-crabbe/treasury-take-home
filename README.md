...
The end to end flow is the user creates validation request and then the server sends back a 200 while the validation process is running (ocr extraction, comparison, etc) and the client continues to poll until the server finishes that validation job (status = completed). Then a result modal shows up showing whether or not it was a success and if it wasn't then it's respective issues.
